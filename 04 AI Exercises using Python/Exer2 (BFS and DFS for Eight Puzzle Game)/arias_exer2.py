...
class EightPuzzleGame(Tk): # Tkinter Classes learned from tkinter documentation http://tkdocs.com/tutorial
    def __init__(self):
        ...
        self.dropDownList = OptionMenu(self.frame, self.dropDownChoice, "BFS", "DFS")
    ...
    def onClickSolve(self, c): #function for using the brute force algorithms
        ...
        self.puzzleData = self.loadPuzzleData() #reloads the puzzle.in data
        self.buttons = self.createButtons(self.frame)
        self.pathCost = 0
    ...
    def BFSearch(self, initPuzzle): #Breadth First Search Algorithm
        ...
        exploredPuzzles.append(frontier[0]["puzzleData"]) #appends first state into explored
        ...
        while(len(frontier)>0): #while loop for BFSearch
            ...
            if(self.checkIfWin(currentState['puzzleData'])):
                ...
                for x in currentState["actions"]:
                    ...
                    if not self.checkIfIn(result['puzzleData'], exploredPuzzles):
                        frontier.append(result) #queues new state into frontier
                        exploredPuzzles.append(result['puzzleData']) #appends puzzledata to explored states
    ...
    def DFSearch(self, initPuzzle): #Almost the same as BFS
        ...
        exploredPuzzles.append(frontier[0]["puzzleData"])

        print("Initial State:")
        self.printState(frontier[0])

        while(len(frontier)>0):
            currentState = frontier.pop() #Only difference between the two algorithms, pops instead of dequeue
            exploredNumber += 1
            print(exploredNumber)
            if(self.checkIfWin(currentState['puzzleData'])):
                return currentState
                ...
                for x in currentState["actions"]:
                    result = self.actionResult(currentState, x)
                    if not self.checkIfIn(result['puzzleData'], exploredPuzzles):
                        frontier.append(result) #pushes the new state
                        exploredPuzzles.append(result['puzzleData'])
